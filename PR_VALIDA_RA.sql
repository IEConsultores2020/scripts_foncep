...
BEGIN
  ...
  IF :OGT_DOCUMENTO_PAGO.CONSECUTIVO IS NOT NULL THEN
    ...
    IF UN_ESTADO IN ('PAC', 'TODO') THEN
      ...
      --QUITAR DESPUES DESDE AQUI
      /*
      :GLOBAL.PAC_VALIDO := 0;    
      PR_CAMBIA_POSICION_ESTADO_ITEM(2
                                    ,:GLOBAL.PAC_VALIDO
                                    ,MI_ESTADO);                                                
      PR_DESPLIEGA_MENSAJE('AL_STOP_1','Verificacion de PAC deshabilitada por paralelos');
      -- HASTA AQUI
      */
    END IF;
    ...
    IF UN_ESTADO IN ('PREDIS', 'TODO') And
      ...
      MI_INDICATIVO_VALIDA_PREDIS := select PK_OGT_OP.FN_OGT_VALOR_BINTABLAS('OPGET',
                                                                      'VALIDA_PREDIS',
                                                                      'INDICATIVO_VALIDA_PREDIS',
                                                                      SYSDATE)
                                                                      from dual
      MI_INDICATIVO_VALIDA_PAC    := SELECT PK_OGT_OP.FN_OGT_VALOR_BINTABLAS('OPGET',
                                                                      'VALIDA_PAC',
      ...
      MI_CUR_REGISTRO             := PK_OGT_OP.FN_OGT_RP_PAGO(:OGT_RELACION_AUTORIZACION.VIGENCIA,
                                      ...
                                      SELECT *
                                      FROM PK_OGT_OP.FN_OGT_RP_PAGO(2025,206,'01','RA',12)
      LOOP
      ...
      --QUITAR DESPUES DESDE AQUI
      /*:GLOBAL.PREDIS_VALIDO := 0;    
      PR_CAMBIA_POSICION_ESTADO_ITEM(1
                                    ,:GLOBAL.PREDIS_VALIDO
                                    ,MI_ESTADO);                                                
      PR_DESPLIEGA_MENSAJE('AL_STOP_1','Verificacion de PREDIS deshabilitada por paralelos');
      */
      -- HASTA AQUI
    Else
      :GLOBAL.PREDIS_VALIDO := 0;
    END IF;
    --EL ESTADO 00011101000 MODIFICA LA POSICION 1, ASIGNANDO ...PREDIS_VALIDO = 0
    --QUEDANDO  00011101000 Y LO RETORNA EN MI_ESTADO
    ...
    IF UN_ESTADO IN ('AA', 'TODO') AND
      ...
      /* FMD 11-03-2003
       NO APLICA PARA VALIDACION DE INCAPACIDADES
            -- VALIDAR SALDO DE ANEXO PATRONAL
            MI_SALDO := OGT_PK_RA.OGT_FN_SALDO_NOMINA(:OGT_DOCUMENTO_PAGO.VIGENCIA
                                                            ,:OGT_DOCUMENTO_PAGO.ENTIDAD
                                                            ,:OGT_DOCUMENTO_PAGO.UNIDAD
                                                            ,:OGT_ANEXO_PATRONAL.ID_PAGAR_A);
            MI_APORTE  := OGT_PK_RA.OGT_FN_SALDO_PATRONAL(:OGT_DOCUMENTO_PAGO.VIGENCIA
                                                            ,:OGT_DOCUMENTO_PAGO.ENTIDAD
                                                            ,:OGT_DOCUMENTO_PAGO.UNIDAD
                                                            ,:OGT_ANEXO_PATRONAL.ID_PAGAR_A);
      
            IF MI_SALDO > MI_APORTE THEN
              PR_DESPLIEGA_MENSAJE('AL_STOP_1','El valor de las incapacidades menos el saldo por tercero '||
                                              'superan el valor del aporte patronal');
              :GLOBAL.ANEXO_VALIDO := 1;                                        
            END IF;
      */
