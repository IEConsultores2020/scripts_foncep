PROCEDURE PR_PLANOS_RA_SHD(una_compania      VARCHAR2,
  ...
  cursor encabezado(un_grupo varchar2) is
    ...
    SELECT TO_CHAR(sysdate, 'yyyymmdd') fdoc,
           ...
           'COP' moneda,
           LPAD(' ', 9) cambio,
           'Nomina '||TO_CHAR(una_fecha_final, 'FMMONTH', 'NLS_DATE_LANGUAGE = Spanish') nrodoc,
           '13 Nomina ' || TO_CHAR(una_fecha_final, 'FMMONTH', 'NLS_DATE_LANGUAGE = Spanish') Cabecera
      FROM dual;
  ...
  LOOP
    ...
    for e in reg40(mi_ra_type.mi_nro_ra) loop
      ...
      mi_szLinea     := 'P' || chr(09) || '40' || chr(09) ||
                        ...
                        ||'Nomina '||TO_CHAR(SYSDATE, 'FMMONTH', 'NLS_DATE_LANGUAGE = Spanish'); --||e.pospres||chr(09)      
      Text_IO.Put_Line(mi_archivo_sap, mi_szLinea);
    end loop;
    mi_cuentareg := 0;
