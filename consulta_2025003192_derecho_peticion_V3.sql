...
from rh_personas p, rh_funcionario f, 
    (
        ...
        WHERE
            ...
            AND hn.brechazado = 0
            AND besdefinitivo = 1
            ---- Siempre que se consulte el historico nomina se debe adicionar estos parametros -------------
            AND (hn.ntipoconcepto IN (2,3,4) OR (hn.ntipoconcepto = 1 and 
                                                 hn.nretroactivo = (select nvl( min( ndCampo0 ), 0 )
                                                                      from rh_historico_nomina
                                                                     Where dInicioperiodo  <= hn.dinicioperiodo
                                                                       and dFinalperiodo   >= hn.dfinalperiodo
                                                                       and nFuncionario     = hn.nfuncionario
                                                                       and nHash            = 812839052)))
            ------------------------------------- 
        GROUP BY
            hn.nfuncionario,
