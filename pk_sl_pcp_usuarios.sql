...
(
  ...
  USUARIO_CREACION  NUMBER(18) not null,
  USUARIO_ACTUALIZA NUMBER(18),
  FECHA_ACTUALIZA   DATE,
  TIPO_USUARIO      VARCHAR2(1) not null,
  INICIALIZA_CLAVE  NUMBER(1) not null,
  CENTRO_COSTO      NUMBER(38) not null
)
;


ALTER TABLE SL_PCP_USUARIOS
ADD PRIMARY KEY (ID);

ALTER TABLE SL_PCP_USUARIOS
ADD CONSTRAINT UNQ_USUARIO UNIQUE(USUARIO);
...
CREATE OR REPLACE PACKAGE pk_sl_pcp_usuarios AS
    ...
    PROCEDURE update_usuario(
        ...
        usuario_actualiza IN SL_PCP_USUARIOS.USUARIO_ACTUALIZA%TYPE,
...
CREATE OR REPLACE PACKAGE BODY pk_sl_pcp_usuarios AS
    ...
    FUNCTION create_usuario(
        ...
        usuario_creacion   IN SL_PCP_USUARIOS.USUARIO_CREACION%TYPE,
        tipo_usuario       IN SL_PCP_USUARIOS.TIPO_USUARIO%TYPE,
        centro_costo       IN SL_PCP_USUARIOS.CENTRO_COSTO%TYPE
    ) RETURN NUMBER IS
        user_id NUMBER;
    BEGIN
        INSERT INTO SL_PCP_USUARIOS (
            ...
            'A',    --estado,
            usuario_creacion, --p_usuario_data.usuario_creacion,
    ...
    BEGIN
        UPDATE SL_PCP_USUARIOS
           SET NOMBRE_USUARIO     = nombre_usuario,
               ESTADO             = estado,
               USUARIO_ACTUALIZA  = usuario_actualiza, --p_usuario_data.usuario_actualiza,
    ...
    BEGIN
        SELECT ID
         ...
         WHERE USUARIO = p_usuario
           ...
           AND ESTADO  = 'A'; -- Asumiendo que 'ACTIVO' es un estado válido

        RETURN v_user_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN -1; -- O un valor específico para indicar usuario/contraseña incorrectos
...
CREATE OR REPLACE PUBLIC SYNONYM PK_SL_PCP_USUARIOS FOR SL.PK_SL_PCP_USUARIOS;
...
REVOKE SELECT, INSERT, UPDATE, DELETE ON SL_PCP_USUARIOS FROM PCP_ADMIN;
