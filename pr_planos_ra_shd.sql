PROCEDURE PR_PLANOS_RA_SHD(una_compania      VARCHAR2,
  ...
  cursor encabezado(un_grupo varchar2) is
    ...
    SELECT TO_CHAR(sysdate, 'yyyymmdd') fdoc,
           ...
           TO_CHAR(SYSDATE, 'mm') periodo,
           'COP' moneda,
           LPAD(' ', 9) cambio,
           'Nom' || decode(un_grupo_ra, '5', 'Func', 'Inv') ||
           TO_CHAR(una_fecha_final, 'yyyymmdd') nrodoc,
           'NomPer' || TO_CHAR(una_fecha_final, 'yyyymmdd') Cabecera
      FROM dual;
  ...
  LOOP
    ...
    for e in reg40(mi_ra_type.mi_nro_ra) loop
      ...
      mi_szLinea     := 'P' || chr(09) || '40' || chr(09) ||
                        ...
                        chr(09) || e.REGISTRO || chr(09) || e.pospres; --||e.pospres||chr(09)      
      Text_IO.Put_Line(mi_archivo_sap, mi_szLinea);
    end loop;
    mi_cuentareg := 0;
    ...
    for r in rubros(mi_ra_type.mi_nro_ra) loop
      ...
      pr_debug_registra(mi_file_debug_handle,'722 Inicia cur_anexos');
      ...
      LOOP
            ...
            LOOP
              ...
              IF un_tipo_ra = '1' THEN
                ...
                mi_szLinea := mi_szLinea || chr(09) || '  ' || mi_var90rete ||
                              ...
                              mi_basertfte || chr(09) || mi_retefuente ||
                              chr(09);
                Text_IO.Put_Line(mi_archivo_sap, mi_szLinea);
        ...
        ELSIF UPPER(mi_descripcion_cc) LIKE '%EMBARGO%' and mi_cc = 7 THEN
          BEGIN
            --FTV PRUEBA 202405 linea para mostrar en caso de error.
        ...
        ELSE
          BEGIN
            ...
            IF un_tipo_ra = '1' THEN
              ...
              mi_consulta := mi_consulta ||
                             'IN (0,1)  AND      c.codigo    not  IN (2,3,4,5) ';
            ELSE
            ...
            mi_linea_ejecutada := 'EXEC_SQL.PARSE mi_consulta ' ||
	          ...
	          pr_debug_registra(mi_file_debug_handle,'1598 mi_tabla_detalle '||mi_tabla_detalle);     
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    ELSE
     ...
     pr_debug_registra(mi_file_debug_handle,'22 Ocurrió un error. mi_cc | MES | linea :' ||
                           mi_cc || ' | ' || SQLERRM() || '| ' ||
                           mi_linea_ejecutada);   
     	if :B_RA.cta_x_nomina = '999999999' then                      
				if text_io.is_open(mi_file_debug_handle) then 
