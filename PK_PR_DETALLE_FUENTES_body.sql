...
--INIRQ126-2009 
---ijo fincion para perno cambiar valor 'por saldo                                
FUNCTION fn_cursor_valFuentes  (una_vigencia   pr_apropiacion.vigencia%TYPE,
                      ...
                      RETURN Pk_Pr_Detalle_Fuentes.cur_c_valFuentes IS
                                                         

c_valFuentes Pk_Pr_Detalle_Fuentes.cur_c_valFuentes;
  BEGIN
  
  IF NVL(un_rp,0)=0 THEN --retorna las fuentes del cdp 
      OPEN c_valFuentes
      FOR 
         SELECT deta.codigo_fuente codfte ,ftes.descripcion descfte, 
                deta.codigo_det_fuente_financ coddet, detalle.descripcion descdet, 
                sum(valor)
        FROM pr_detalle_fuentes_cdp  deta,
             pr_fuentes_financiacion ftes,
             pr_detalle_fuentes      detalle
        WHERE detalle.codigo_fuentes_financiacion=ftes.codigo 
        AND ftes.codigo=deta.codigo_fuente
        AND deta.vigencia=detalle.vigencia
        AND deta.clasificacion=detalle.clasificacion
        AND deta.codigo_fuente=detalle.codigo_fuentes_financiacion
        AND deta.codigo_det_fuente_financ=detalle.consecutivo_fuente
        AND deta.vigencia=una_vigencia
        AND deta.codigo_compania=una_compania
        AND deta.codigo_unidad_ejecutora=una_unidad
        AND deta.rubro_interno=un_rubro
        AND    deta.numero_disponibilidad=un_cdp
        GROUP by deta.codigo_fuente,ftes.descripcion, 
                 deta.codigo_det_fuente_financ, detalle.descripcion;
    ELSE  --retorna las fuentes del rp 
      OPEN c_valFuentes
      FOR 
       SELECT deta.codigo_fuente codfte ,ftes.descripcion descfte, 
       deta.codigo_det_fuente_financ coddet, detalle.descripcion descdet, sum(valor)
        FROM pr_detalle_fuentes_rp   deta,
             pr_fuentes_financiacion ftes,
             pr_detalle_fuentes      detalle
        WHERE detalle.codigo_fuentes_financiacion=ftes.codigo 
        AND ftes.codigo=deta.codigo_fuente
        ...
        AND deta.rubro_interno=un_rubro
        AND    deta.numero_disponibilidad=un_cdp
        AND deta.NUMERO_REGISTRO=un_rp
        GROUP by deta.codigo_fuente,ftes.descripcion, 
       deta.codigo_det_fuente_financ, detalle.descripcion;

    END IF;
    RETURN (c_valFuentes);            
 END fn_cursor_valFuentes;                                
--FINRQ126-2009  
---FIN 2025002524 
