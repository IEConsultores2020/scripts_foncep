...
2025003170	V4 	20250821 FTORRESV 
	       	En las líneas 96 a 102 figura via de pago M, en este caso el pago es transferencia y no debe aparecer ningún tipo de via de pago.					
		      En las líneas 103 y 104 no se coloca el nit de las entidades y en su lugar en la columna 6 y ss el código de cuenta 1000466363.
		      Revisar el código del Banco de Bogota es 001 y en el plano aparece 000, 
          El código del Banco NU es 809 en el aplano aparece 180, 
...
2025005004  V6   20251212 FTORRESV a
          ...
          En la información presupuestal está tomando los valores de la RA pero solamente los de la columna Regimen Antiguo 
            y debe tomar es el total del Rubro.
          Se debe incluir entre las columnas 13 a 15, en las líneas donde hay giro a cuenta bancaria, el código 999
*/
  ...
  mi_tipo_funcionarios bintablas.resultado%type;  ---varchar 2025003680 Para generar ra seguridad social
...
BEGIN
  ...
  BEGIN
    IF GET_APPLICATION_PROPERTY(USER_INTERFACE) = 'WEB' THEN
      ...
      If Get_Application_Property(OPERATING_SYSTEM) Like '%WIN%' Then
        --ini 2026001329 
        mi_ruta_archivo     := mi_www_path || '\' || mi_archivo_planosap;
        mi_archivo_sap := text_io.fopen(mi_ruta_archivo,'w');
        --mi_archivo_sap := text_io.fopen(mi_www_path || '\' ||mi_archivo_planosap,'w');
        --fin 2026001329
      Else
        --ini 2026001329 
        mi_ruta_archivo     := mi_www_path || '/' || mi_archivo_planosap;
    ...
    ELSE
      --ini 2026001329 
      mi_ruta_archivo := 'c:\' || mi_archivo_planosap;
      mi_archivo_sap := text_io.fopen(mi_ruta_archivo, 'w');
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      IF mi_sqlcode = -302000 then
        ...
        END LOOP;
      END IF;
      --ini 2026001329 se adiciona mi_ruta_archivo
      pr_despliega_mensaje('AL_STOP_1',
                           '1 Ocurrió un error mi_ruta_archivo: ' ||mi_ruta_archivo||' .'|| SQLERRM() || ' ' ||
                           SQLCODE());
      mi_err := 1;
      RETURN;
  END;
  mi_condicion_pago := una_compania || '1';
--	pr_debug_registra(mi_file_debug_handle,'mi_condicion_pago: '||mi_condicion_pago);  2025004504 
       ...
       /* mi_linea_ejecutada := 'FETCH cur_anexos mi_cc||mi_archivo_plano||mi_tabla_detalle :' ||
            ...
            LOOP
              ...
              IF un_tipo_ra = '1' THEN
                ...
                mi_szLinea := mi_szLinea || chr(09) || mi_condicion_pago ||
                              ...
                              rpad(mi_funcionario_type.mi_numero_cuenta,20,' ') || chr(09) ||
        ...
        ELSIF UPPER(mi_descripcion_cc) LIKE '%EMBARGO%' and mi_cc = 6 /*FTv*/ THEN
          BEGIN
            ...
            LOOP
              ...
              mi_szLinea := 'P' || chr(09) || mi_codigo || chr(09) ||
                            chr(09) || 'NIT' || chr(09) ||
                            rpad('1000466363', 12, ' ') || chr(09) || --rpad(mi_nit_agrario, 12, ' ') || chr(09) ||   --2025003170  
          ...
          --2025004504 Se adiciona /
            web.show_document(mi_pathweb_ra||'/'||mi_archivo_planoemb,'_blank');
          
            ------------ otros embargos se adicionan al archivo de Nomina 20200708
            --INI 2025004504 se quitan comentarios
            mi_archivo_planoemb:=una_compania||'_'||mi_vigencia||'_'||mi_mes||'_'||to_char(sysdate,'yyyymmddhhmiss')||'_'||mi_ra_type.mi_nro_ra||'_'||'embargosnba.txt';
                   If Get_Application_Property(OPERATING_SYSTEM) Like '%WIN%' Then
        ...
        ELSE
          BEGIN
            ...
            IF (mi_tabla_detalle LIKE '%NOMBRE%' OR
               mi_tabla_detalle = 'ENTIDAD_BENEFICIARIOS') THEN
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,1,mi_concepto,30);
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,2,mi_tercero,30);
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,3,mi_valor);
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,4,mi_valor_saldo);
            ELSE
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,1,mi_tercero,30);
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,2,mi_valor);
              EXEC_SQL.DEFINE_COLUMN(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,3,mi_valor_saldo);
            END IF;
            ...
            WHILE EXEC_SQL.FETCH_ROWS(EXEC_SQL.DEFAULT_CONNECTION,
              ...
              IF (mi_tabla_detalle LIKE '%NOMBRE%' OR
                 mi_tabla_detalle = 'ENTIDAD_BENEFICIARIOS') THEN
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,1,mi_concepto);
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,2,mi_tercero);
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,3,mi_valor);
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,4,mi_valor_saldo);
              ELSE
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,1,mi_tercero);
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,2,mi_valor);
                EXEC_SQL.COLUMN_VALUE(EXEC_SQL.DEFAULT_CONNECTION,mi_cursor,3,mi_valor_saldo);
              END IF;
