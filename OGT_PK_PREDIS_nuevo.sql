create or replace PACKAGE BODY     Ogt_Pk_Predis IS
  --GLP2025002642 20260704 Alinear predis CUD.
  -- 06-09-2004. Sergio.
  ...
  --INI GLP2025002642
  FUNCTION Ogt_Fn_Valor_aportes_Mes(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
    un_rubro_interno     NUMBER) RETURN NUMBER IS
    mi_desc_rubro               pr_rubro.descripcion%TYPE;
    valor_aportes_empleado_mes  NUMBER;
  begin
    
    select descripcion
      into mi_desc_rubro
      from pr_v_rubros
     where  interno_rubro = un_rubro_interno
       and vigencia       = una_vigencia;

    if nvl(mi_desc_rubro,'') = 'Sueldo básico' then
      --Centros de costos: 5 es APORTE SALUD, 1285 es APORTE REGIMEN SOLIDARIDAD, 1267 es APORTE PENSION
      --Pendiente validar que la OP se encuentre enviada a tesoreria.
        select sum(valor)
          into valor_aportes_empleado_mes
        from ogt_centro_costos
        where entidad         =  una_entidad
        and unidad_ejecutora  = una_unidad_ejecutora
        and vigencia          = una_vigencia
        and extract(month from fecha_desde) = un_mes
        --and consecutivo=10
    ...
    return valor_aportes_empleado_mes;
  exception
    when others then
      return 0;
  end;
...
/*
  FUNCTION Ogt_Fn_pension_pub_empleado(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
  ...
  begin
    ...
    select interno_rubro
      into mi_desc_rubro
      from pr_v_rubros
     where descripcion = un_rubro_interno
       and vigencia = :vigencia;

    if mi_desc_rubro = 'Sueldo básico' then
      --Centros de costos: 5 es APORTE SALUD, 1285 es APORTE REGIMEN SOLIDARIDAD, 1267 es APORTE PENSION
      --Pendiente validar que la OP se encuentre enviada a tesoreria.
      valor_aportes_empleado_mes := 0;
    else 
  ...
  exception
    valor_aportes_empleado_mes := 0;
  end Ogt_Fn_pension_pub_empleado;

  FUNCTION Ogt_Fn_pension_priv_empleado(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
  ...
  exception
    valor_aportes_empleado_mes := 0;
  end Ogt_Fn_pension_priv_empleado;

  FUNCTION Ogt_Fn_salud_pub_empleado(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
  ...
  exception
    valor_aportes_empleado_mes := 0;
  end Ogt_Fn_salud_pub_empleado;

  FUNCTION Ogt_Fn_salud_priv_empleado(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
  ...
  exception
    valor_aportes_empleado_mes := 0;
  end Ogt_Fn_salud_priv_empleado;

  FUNCTION Ogt_Fn_fndo_sldrdad_empleado(
    una_vigencia         NUMBER,
    una_entidad          VARCHAR2,
    una_unidad_ejecutora VARCHAR2,
    un_mes               NUMBER,
  ...
  exception
    valor_aportes_empleado_mes := 0;
  end Ogt_Fn_fndo_sldrdad_empleado;
  */
  --FIN GLP2025002642
  ...
  FUNCTION Ogt_Fn_Valor_Mes(una_vigencia         NUMBER,
                            ...
                            un_rubro_interno     NUMBER) RETURN NUMBER IS
    --INI GLP2025002642
    valor_aportes_empleado_mes    number := 0;
    valor_aportes_empleado_mesant number := 0;
    valor_pension_publica       number := 0;
    valor_pension_privada       number := 0;
    valor_salud_publica         number := 0;
    valor_salud_privada         number := 0;
    valor_fondo_solidaridad     number := 0;
    --FIN GLP2025002642
  BEGIN
    ...
    --INI GLP2025002642 Alineación CUD
    if to_date('01/'||un_mes||'/'||una_vigencia,'dd/mm/yyyy') >= to_date('01/04/2025','dd/mm/yyyy') then
      valor_aportes_empleado_mes := Ogt_Fn_Valor_aportes_Mes(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
      --1.Se resta al rubro SUELDO BASICO los pagos de aportes del empleado de la nómina del mes
      valor_aportes_empleado_mes := Ogt_Fn_Valor_aportes_Mes(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
      --2. Se suma aportes de salud, aportes de pensión y aportes de solidaridad pagados el mes anterior, a los aportes del mes actual
      --Pendiente, si afecta rubro difente a sueldo básico. En ogt_diligenciamiento cruzando centro de costo con anexos nomina se trae el tercero y luego validar si es fondo privado o publico,
      /*if un_mes=1 then
        valor_aportes_empleado_mesant := 0;
      else
        valor_aportes_empleado_mesant := Ogt_Fn_Valor_aportes_Mes(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes-1, un_rubro_interno);  
      end if;*/
    end if;

    /*--Pendiente Si el rubro corresponde a la descripción, retorna el valor de lo contrario retorna cero
    valor_pension_publica := Ogt_Fn_pension_pub_empleado(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
    valor_pension_privada := Ogt_Fn_pension_priv_empleado(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);   
    valor_salud_publica   := Ogt_Fn_salud_pub_empleado(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
    valor_salud_privada   := Ogt_Fn_salud_priv_empleado(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
    valor_fondo_solidaridad  := Ogt_Fn_fndo_sldrdad_empleado(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
    */
    --FIN GLP2025002642


    RETURN (NVL(mi_valor_op, 0) + NVL(mi_valor_ra, 0)
      --INI GLP2025002642 El valor a restar o a sumar o restar al rubro aplica si el rubro corresponde a la descripción
      - valor_aportes_empleado_mes
      --+ valor_aportes_empleado_mesant
      + valor_pension_publica
      + valor_pension_privada
      + valor_salud_publica
      + valor_salud_privada
      + valor_fondo_solidaridad
    );
      --FIN GLP2025002642
  ...
  BEGIN
    ...
    --INI GLP2025002642
    if to_date('01/'||un_mes||'/'||una_vigencia,'dd/mm/yyyy') >= to_date('01/04/2026','dd/mm/yyyy') then
      valor_aportes_empleado_mes := Ogt_Fn_Valor_aportes_Mes(una_vigencia, una_entidad, una_unidad_ejecutora, un_mes, un_rubro_interno);  
    end if;
    --FIN GLP2025002642
