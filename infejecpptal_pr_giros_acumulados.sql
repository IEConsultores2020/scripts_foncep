  ...
  -- Valor Total de OP registradas acumulados
      ...
      select --nvl(sum(nvl(pr_orden_de_pago_registro.valor,0)),0) total_valor_op
        --into mi_valor_op
       ...
       where ( pr_orden_de_pago_registro.consecutivo_orden = pr_orden_de_pago.consecutivo_orden
         ...
         and to_char(pr_orden_de_pago.fecha_registro,'YYYY') = :vigencia
         and to_char(pr_orden_de_pago.fecha_registro,'MM') <= :p_mes;
         --prod 7845000  test 
  -- Valor de Ordenes del mes anuladas acumuladas

      select nvl(sum(nvl(pr_orden_de_pago_registro.valor,0)),0)  op_anuladas
       ...
       where ( pr_orden_de_pago_registro.consecutivo_orden = pr_orden_de_pago.consecutivo_orden
         ...
         and ( pr_orden_de_pago_registro.numero_orden in (
                     ...
                     where vigencia = :vigencia
                        ...
                        and to_char(fecha_registro,'MM') <= :p_mes
                  )
    ...
    -- Ajustes
      ...
      select nvl(sum(nvl(pr_reintegro_ajustes_rubro.valor,0)),0)  ajustes
       ...
       where ( pr_reintegro_ajustes_rubro.consecutivo_ajuste = pr_reintegro_ajustes.consecutivo_ajuste
         ...
         and to_char(pr_reintegro_ajustes.fecha_registro,'mm') <= :p_mes
         and pr_reintegro_ajustes.tipo_movimiento = 'AJUSTE';

    -- Reintegro

      select nvl(sum(nvl(pr_reintegro_ajustes_rubro.valor,0)),0) reintegros
    ...
    select *
    from pr_reintegro_ajustes_rubro 
    where vigencia=2025
    and rubro_interno = 1491
   ;
/*
   Favor revisar y verificar la solución.
   Para solución definitiva debe modificarse la funcionalidad de reembolso, para lo cual el area de presupuestos debe generar una nueva solicitud.
*/
