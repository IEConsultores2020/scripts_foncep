--CURSOR cur_cuenta_afectable IS
    ...
    --select * 
     ...
     WHERE NVL(top.bin_tipo_cuenta,'0')           = 'FD' --mi_tipo_cuenta_bancaria
       AND NVL(top.cote_id,'0')                 = '00-02-37-19-00-00-00' --mi_concepto_ingreso
       ...
       AND top.titr_nombre = DECODE('LEGALIZACION PORTALP' /*mi_tipo_transaccion_contable*/,'NO_AJUSTE',top.titr_nombre,'LEGALIZACION PORTALP' /*mi_tipo_transaccion_contable*/)  
      --cambia XYX por SISTEMA FINANCIERO
       AND top.titr_nombre NOT IN (DECODE('LEGALIZACION PORTALP'/*mi_tipo_transaccion_contable*/,'NO_AJUSTE','AJUSTE DE INGRESO','NO_AJUSTE'))
